 ...
 */
const getSetUnityCatalogTagsDtos =
	({ ddlProvider }) =>
	({ entityData, name }) => {
 ...
 * @returns {GetAlterScriptDtoFunction}
 */
const getUnsetUnityCatalogTagsScriptsDtosFrom =
	({ ddlProvider }) =>
	({ entityData, name }) => {
		const unityTags = entityData?.role?.compMod?.unityCatalogTags || {};
		const newUnityTags = unityTags.new || [];
 ...
 * @returns {GetAlterScriptDtoFunction}
 */
const getModifyUnityCatalogTagsScriptDtos =
	({ ddlProvider }) =>
	({ entityData, name }) => {
 ...
 * @returns {GetAlterScriptDtoFunction}
 */
const getSetUnitySchemaTagsDtos =
	({ ddlProvider }) =>
	({ entityData, name }) => {
 ...
 * @returns {GetAlterScriptDtoFunction}
 */
const getUnsetUnitySchemaTagsScriptsDtosFrom =
	({ ddlProvider }) =>
	({ entityData, name }) => {
		const unityTags = entityData?.role?.compMod?.unitySchemaTags || {};
		const newUnityTags = unityTags.new || [];
 ...
 * @returns {GetAlterScriptDtoFunction}
 */
const getModifyUnitySchemaTagsScriptDtos =
	({ ddlProvider }) =>
	({ entityData, name }) => {
