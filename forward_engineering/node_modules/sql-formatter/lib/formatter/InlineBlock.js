...
var InlineBlock = /*#__PURE__*/function () {
  ...
  _createClass(InlineBlock, [{
    ...
    value: function betweenWidth(node) {
      return node.betweenToken.text.length + this.inlineWidth(node.expr1) + node.andToken.text.length + this.inlineWidth(node.expr2);
    ...
    value: function isForbiddenToken(token) {
      return (0, _token.isLogicalOperator)(token) || token.type === _token.TokenType.LINE_COMMENT || token.type === _token.TokenType.BLOCK_COMMENT || token.type === _token.TokenType.CASE // CASE cannot have inline blocks
      ;
    }
  }]);
