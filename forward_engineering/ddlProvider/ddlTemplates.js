module.exports = {
	createView:
		'CREATE${orReplace}${global}${temporary}${materialized} VIEW${ifNotExists} ${name} ${columnList}${schemaBinding}${partitioningKeyClause}${clusteringKeyClause}${comment}${tablePropertyStatements}${scheduleClause}${query};${viewUnityTagsStatements}',
	...
	addPkConstraint:
		'ALTER TABLE ${tableName} ADD CONSTRAINT ${constraintName} PRIMARY KEY(${pkColumns})${constraintOptions};',

	dropPkConstraint: 'ALTER TABLE ${tableName} DROP PRIMARY KEY IF EXISTS ${dropPkOption};',

	addFkConstraint:
		'ALTER TABLE ${childTableName} ADD${fkConstraintName} FOREIGN KEY (${childColumns}) REFERENCES ${parentTableName}(${parentColumns});',

	addInlineFkConstraint:
		'${fkConstraintName}FOREIGN KEY (${childColumns}) REFERENCES ${parentTableName}(${parentColumns})',
